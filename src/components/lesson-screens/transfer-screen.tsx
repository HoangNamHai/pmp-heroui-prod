...
export function TransferScreen({ screen, onAnswer, answeredQuestions }: TransferScreenProps) {
  ...
  const handleSalienceRatingSubmit = () => {
    ...
    dimensions.forEach((dimension: any) => {
      if (salienceRatings[dimension.id] === dimension.correct) correctCount++;
    });
  ...
  const handleCategorizationSubmit = () => {
    ...
    items.forEach((item: any) => {
      if (categoryAnswers[item.id] === item.correct) correctCount++;
    });
    const isCorrect = correctCount === items.length;
    const partialPoints = Math.round((correctCount / items.length) * currentQuestion.points);
  ...
  const handleCalculationSubmit = () => {
    ...
    fields.forEach((field: any) => {
      ...
      if (Math.abs(userValue - correctValue) < 0.01) correctCount++;
    });
  ...
  const handleUserStorySubmit = () => {
    ...
    const isCorrect = storyParts.who === correct.who && storyParts.what === correct.what && storyParts.why === correct.why;
  ...
  const handleAcToggle = (optionId: string) => {
    if (answeredQuestions[currentQuestion?.id]) return;
    setAcSelections(prev => prev.includes(optionId) ? prev.filter(id => id !== optionId) : [...prev, optionId]);
  };
  ...
  const handleInvestSubmit = () => {
    ...
    criteria.forEach((criterion: any) => {
      ...
      if (userAnswer === criterion.correct) correctCount++;
    });
  ...
  const handleSelectionToggle = (itemId: string) => {
    ...
    setSelectedItems(prev => {
      ...
      if (prev.length < capacity) return [...prev, itemId];
      return prev;
    });
  };
  ...
  return (
    <ScrollView className="flex-1" contentContainerClassName="px-5 py-4" showsVerticalScrollIndicator={false}>
      ...
      {currentQuestion.type === 'salience_rating' && (
        <View className="gap-4">
          {currentQuestion.instruction && <AppText className="text-muted text-sm mb-2">{currentQuestion.instruction}</AppText>}
              ...
              <View key={dimension.id} className="gap-2">
                ...
                <View className="flex-row gap-2">
                  {['High', 'Low'].map((value) => {
                    ...
                    return (
                      <Pressable key={value} onPress={() => handleSalienceRatingChange(dimension.id, value)} disabled={!!isAnswered} className="flex-1">
                        <View className={cn('p-3 rounded-xl border-2 items-center', !showResult && !isSelected && 'border-divider bg-content1', !showResult && isSelected && 'border-accent bg-accent/10', showResult && isSelected && isCorrect && 'border-success bg-success/10', showResult && isSelected && !isCorrect && 'border-danger bg-danger/10', showResult && !isSelected && dimension.correct === value && 'border-success/50 bg-success/5')}>
                          <AppText className={cn('font-medium', !showResult && isSelected && 'text-accent', !showResult && !isSelected && 'text-foreground', showResult && isSelected && isCorrect && 'text-success', showResult && isSelected && !isCorrect && 'text-danger', showResult && !isSelected && dimension.correct === value && 'text-success')}>{value}</AppText>
                        </View>
                      </Pressable>
                    );
                  })}
          ...
          {!isAnswered && <Button color="primary" className="mt-4" onPress={handleSalienceRatingSubmit} isDisabled={Object.keys(salienceRatings).length < (currentQuestion.dimensions || []).length}>Submit Answer</Button>}
        </View>
      )}
      ...
      {currentQuestion.type === 'categorization' && (
        <View className="gap-4">
              ...
              <View key={item.id} className="gap-2">
                ...
                <View className="flex-row flex-wrap gap-2">
                  {(currentQuestion.categories || []).map((category: string) => {
                    ...
                    return (
                      <Pressable key={category} onPress={() => handleCategorySelect(item.id, category)} disabled={!!isAnswered}>
                        <View className={cn('px-4 py-2 rounded-xl border-2', !showResult && !isSelected && 'border-divider bg-content1', !showResult && isSelected && 'border-accent bg-accent/10', showResult && isSelected && isCorrect && 'border-success bg-success/10', showResult && isSelected && !isCorrect && 'border-danger bg-danger/10', showResult && !isSelected && item.correct === category && 'border-success/50 bg-success/5')}>
                          <AppText className={cn('text-sm', !showResult && isSelected && 'text-accent font-medium', !showResult && !isSelected && 'text-foreground', showResult && isSelected && isCorrect && 'text-success font-medium', showResult && isSelected && !isCorrect && 'text-danger', showResult && !isSelected && item.correct === category && 'text-success')}>{category}</AppText>
                        </View>
                      </Pressable>
                    );
                  })}
          ...
          {!isAnswered && <Button color="primary" className="mt-4" onPress={handleCategorizationSubmit} isDisabled={Object.keys(categoryAnswers).length < (currentQuestion.items || []).length}>Submit Answer</Button>}
        </View>
      )}
          ...
          {(currentQuestion.fields || []).map((field: any) => {
            ...
            return (
              <View key={field.id} className="gap-2">
                ...
                <TextInput placeholder="Enter value..." value={userAnswer} onChangeText={(value) => handleCalculationChange(field.id, value)} keyboardType="numeric" editable={!isAnswered} className={cn('p-4 rounded-xl border-2 text-foreground text-base', !showResult && 'border-divider bg-content1', showResult && isCorrect && 'border-success bg-success/10', showResult && !isCorrect && 'border-danger bg-danger/10')} placeholderTextColor="#999" />
                {showResult && !isCorrect && <AppText className="text-success text-sm">Correct: {field.correct}</AppText>}
              </View>
            );
          })}
          {!isAnswered && <Button color="primary" className="mt-4" onPress={handleCalculationSubmit} isDisabled={Object.keys(calculationAnswers).length < (currentQuestion.fields || []).length}>Submit Answer</Button>}
        </View>
      )}
      ...
      {currentQuestion.type === 'user_story_builder' && (
        <View className="gap-4">
          ...
          <View className="gap-2">
            ...
            <View className="flex-row flex-wrap gap-2">
              {(currentQuestion.whoOptions || []).map((option: string) => {
                ...
                return (
                  <Pressable key={option} onPress={() => handleStoryPartChange('who', option)} disabled={!!isAnswered}>
                    <View className={cn('px-4 py-2 rounded-xl border-2', !showResult && !isSelected && 'border-divider bg-content1', !showResult && isSelected && 'border-accent bg-accent/10', showResult && isSelected && isCorrect && 'border-success bg-success/10', showResult && isSelected && !isCorrect && 'border-danger bg-danger/10')}>
                      <AppText className={cn('text-sm', !showResult && isSelected && 'text-accent font-medium', !showResult && !isSelected && 'text-foreground', showResult && isSelected && isCorrect && 'text-success font-medium', showResult && isSelected && !isCorrect && 'text-danger')}>{option}</AppText>
                    </View>
                  </Pressable>
                );
              })}
          ...
          <View className="gap-2">
            ...
            <View className="flex-row flex-wrap gap-2">
              {(currentQuestion.whatOptions || []).map((option: string) => {
                ...
                return (
                  <Pressable key={option} onPress={() => handleStoryPartChange('what', option)} disabled={!!isAnswered}>
                    <View className={cn('px-4 py-2 rounded-xl border-2', !showResult && !isSelected && 'border-divider bg-content1', !showResult && isSelected && 'border-accent bg-accent/10', showResult && isSelected && isCorrect && 'border-success bg-success/10', showResult && isSelected && !isCorrect && 'border-danger bg-danger/10')}>
                      <AppText className={cn('text-sm', !showResult && isSelected && 'text-accent font-medium', !showResult && !isSelected && 'text-foreground', showResult && isSelected && isCorrect && 'text-success font-medium', showResult && isSelected && !isCorrect && 'text-danger')}>{option}</AppText>
                    </View>
                  </Pressable>
          ...
          <View className="gap-2">
            ...
            <View className="flex-row flex-wrap gap-2">
              {(currentQuestion.whyOptions || []).map((option: string) => {
                ...
                return (
                  <Pressable key={option} onPress={() => handleStoryPartChange('why', option)} disabled={!!isAnswered}>
                    <View className={cn('px-4 py-2 rounded-xl border-2', !showResult && !isSelected && 'border-divider bg-content1', !showResult && isSelected && 'border-accent bg-accent/10', showResult && isSelected && isCorrect && 'border-success bg-success/10', showResult && isSelected && !isCorrect && 'border-danger bg-danger/10')}>
                      <AppText className={cn('text-sm', !showResult && isSelected && 'text-accent font-medium', !showResult && !isSelected && 'text-foreground', showResult && isSelected && isCorrect && 'text-success font-medium', showResult && isSelected && !isCorrect && 'text-danger')}>{option}</AppText>
                    </View>
                  </Pressable>
          ...
          {!isAnswered && <Button color="primary" className="mt-4" onPress={handleUserStorySubmit} isDisabled={!storyParts.who || !storyParts.what || !storyParts.why}>Submit Answer</Button>}
        </View>
      )}
        ...
        <View className="gap-4">
          ...
          <View className="gap-3">
            {(currentQuestion.options || []).map((option: any) => {
              ...
              const showResult = isAnswered;
              return (
                <Pressable key={option.id} onPress={() => handleAcToggle(option.id)} disabled={!!isAnswered}>
                  <View className={cn('p-4 rounded-2xl border-2', !showResult && !isSelected && 'border-divider bg-content1', !showResult && isSelected && 'border-accent bg-accent/10', showResult && isSelected && option.isTestable && 'border-success bg-success/10', showResult && isSelected && !option.isTestable && 'border-danger bg-danger/10', showResult && !isSelected && option.isTestable && 'border-success/50 bg-success/5')}>
                    <View className="flex-row items-center gap-3">
                      <View className={cn('w-6 h-6 rounded border-2 items-center justify-center', !showResult && !isSelected && 'border-muted', !showResult && isSelected && 'border-accent bg-accent', showResult && option.isTestable && 'border-success bg-success', showResult && !option.isTestable && isSelected && 'border-danger bg-danger')}>
                        {(isSelected || (showResult && option.isTestable)) && <StyledFeather name="check" size={14} className="text-white" />}
                      </View>
                      <AppText className={cn('flex-1 text-base', showResult && option.isTestable && 'text-success', showResult && isSelected && !option.isTestable && 'text-danger', !showResult && 'text-foreground')}>{option.text}</AppText>
                    </View>
                  </View>
                </Pressable>
              );
          ...
          {!isAnswered && <Button color="primary" className="mt-4" onPress={handleAcSubmit} isDisabled={acSelections.length === 0}>Submit Answer</Button>}
        </View>
      )}
        ...
        <View className="gap-4">
          ...
          <View className="gap-3">
            {(currentQuestion.criteria || []).map((criterion: any) => {
              ...
              return (
                <Pressable key={criterion.id} onPress={() => handleInvestToggle(criterion.id)} disabled={!!isAnswered}>
                  <View className={cn('p-4 rounded-2xl border-2', !showResult && 'border-divider bg-content1', showResult && isCorrect && 'border-success bg-success/10', showResult && !isCorrect && 'border-danger bg-danger/10')}>
                    <View className="flex-row items-center gap-3">
                      <View className={cn('w-6 h-6 rounded border-2 items-center justify-center', !showResult && !isChecked && 'border-muted', !showResult && isChecked && 'border-accent bg-accent', showResult && isCorrect && 'border-success bg-success', showResult && !isCorrect && 'border-danger bg-danger')}>
                        {isChecked && <StyledFeather name="check" size={14} className="text-white" />}
                      </View>
                      <View className="flex-1">
                        <AppText className={cn('font-semibold mb-1', showResult && isCorrect && 'text-success', showResult && !isCorrect && 'text-danger', !showResult && 'text-foreground')}>{criterion.label}</AppText>
          ...
          {!isAnswered && <Button color="primary" className="mt-4" onPress={handleInvestSubmit}>Submit Answer</Button>}
        </View>
      )}
          ...
          {(currentQuestion.blanks || []).map((blank: any, idx: number) => {
            ...
            return (
              <View key={blank.id} className="gap-2">
                ...
                <TextInput placeholder="Type your answer..." value={userAnswer} onChangeText={(value) => handleFillInChange(blank.id, value)} editable={!isAnswered} className={cn('p-4 rounded-xl border-2 text-foreground text-base', !showResult && 'border-divider bg-content1', showResult && isCorrect && 'border-success bg-success/10', showResult && !isCorrect && 'border-danger bg-danger/10')} placeholderTextColor="#999" />
                {showResult && !isCorrect && <AppText className="text-success text-sm">Correct: {blank.correct}</AppText>}
              </View>
            );
          })}
          {!isAnswered && <Button color="primary" className="mt-4" onPress={handleFillInSubmit} isDisabled={Object.keys(fillInAnswers).length < (currentQuestion.blanks || []).length}>Submit Answer</Button>}
        </View>
      )}
      ...
      {currentQuestion.type === 'selection' && (
        <View className="gap-4">
          {currentQuestion.instruction && <AppText className="text-muted text-sm mb-2">{currentQuestion.instruction}</AppText>}
          <View className="flex-row items-center justify-between">
            <AppText className="text-muted text-sm">Select items (capacity: {currentQuestion.capacity || 5})</AppText>
            <AppText className={cn('text-sm font-medium', selectedItems.length <= (currentQuestion.capacity || 5) ? 'text-accent' : 'text-danger')}>{selectedItems.length} / {currentQuestion.capacity || 5}</AppText>
          </View>
          <View className="gap-3">
            {(currentQuestion.items || []).map((item: any) => {
              ...
              const capacity = currentQuestion.capacity || 5;
              return (
                <Pressable key={item.id} onPress={() => handleSelectionToggle(item.id)} disabled={isAnswered || (!isSelected && selectedItems.length >= capacity)}>
                  <View className={cn('p-4 rounded-2xl border-2', !showResult && !isSelected && 'border-divider bg-content1', !showResult && isSelected && 'border-accent bg-accent/10', showResult && isSelected && item.isCorrect && 'border-success bg-success/10', showResult && isSelected && !item.isCorrect && 'border-danger bg-danger/10', showResult && !isSelected && item.isCorrect && 'border-success/50 bg-success/5', !isAnswered && !isSelected && selectedItems.length >= capacity && 'opacity-50')}>
                    <View className="flex-row items-center gap-3">
                      <View className={cn('w-6 h-6 rounded border-2 items-center justify-center', !showResult && !isSelected && 'border-muted', !showResult && isSelected && 'border-accent bg-accent', showResult && item.isCorrect && 'border-success bg-success', showResult && !item.isCorrect && isSelected && 'border-danger bg-danger')}>
                        {(isSelected || (showResult && item.isCorrect)) && <StyledFeather name="check" size={14} className="text-white" />}
                      </View>
                      <AppText className={cn('flex-1 text-base', showResult && item.isCorrect && 'text-success', showResult && isSelected && !item.isCorrect && 'text-danger', !showResult && 'text-foreground')}>{item.text}</AppText>
                    </View>
                  </View>
                </Pressable>
              );
          ...
          {!isAnswered && <Button color="primary" className="mt-4" onPress={handleSelectionSubmit} isDisabled={selectedItems.length === 0}>Submit Answer</Button>}
        </View>
      )}
